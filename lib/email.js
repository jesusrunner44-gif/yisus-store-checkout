...
<div class="card"><h2 style="margin-top:0;">Pago</h2><p>Estado: <strong>{{payment_status}}</strong><br>Proveedor: {{payment_provider}}<br>ID transacción: {{provider_transaction_id}}<br>Método: {{payment_method}}</p></div>
...
export async function sendOrderEmails({ order, payment, provider, resendApiKey }) {
  const resend = new Resend(resendApiKey);

  const resolvedProvider = provider || order.payment_provider || 'mercadopago';
  const providerLabel = resolvedProvider === 'wompi' ? 'Wompi' : 'Mercado Pago';
